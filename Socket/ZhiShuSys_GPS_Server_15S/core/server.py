...
class TCPRequestHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            try:
                gps_datas = self.request.recv(SOCKET_DATA_MAX).strip()
                if gps_datas:
                    cur_time = datetime.now()
                    insert_values = "insert into %s values " % gps_table
                    gps_count = len(gps_datas.split(","))
                    log.info("gps server receive %s data: %d" % (client, gps_count))
                    for gps in gps_datas.split(","):
                        try:
                            value = str(TCPRequestHandler.gps_parse(gps, cur_time))
                            insert_values += value + ","
                        except Exception as e:
                            continue
                    try:
                        ...
                        emsg = "TCPRequestHandler dbhandle.insert is error: %s" % e.message
                        log.error(emsg)
            except Exception as e:

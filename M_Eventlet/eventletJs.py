...
def tableTransferToJson(conn, popType, sqlDate, sqlPeriod, weekType, qysm, path_new):
    cursor = conn.cursor()
    for wt in weekType:
        for sqld in sqlDate:
            for pop in popType:
                ...
                for sqlh in range(0, 24, 1):
                    fw.write('"%d":[' % sqlh)
                    # if qybh == 'xqbh':
                    sql = "select %s,sum(%s) from population_hour_avg_popdensity where data_month='%s' and data_hour='%s' and workday = '%s' and %s!='' GROUP BY %s"\
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_js\mteventlet'
    ...
    for qybh, qysm in sqlJtqy.items():
        ...
        if conn:
            print info
            pars = (conn, popType, sqlDate, sqlPeriod, weekType, qysm, path_new)
            pool.spawn(tableTransferToJson, conn, popType, sqlDate, sqlPeriod, weekType, qysm, path_new)

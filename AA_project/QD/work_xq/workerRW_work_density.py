...
def tableTransferToJson(conn, work, path):
    ...
    for qysm, qymj in work.items():
        ...
        js = os.path.join(path_new, 'workRWdensity.js')
        ...
        fw.write("\n")
        cursor = conn.cursor()
        sql = 'select %s,sum(workder),%s from qd_workerRW WHERE len(%s)>0 group by %s, %s' % (qysm, qymj, qysm, qysm, qymj)
        print 'sql : %s' % sql
        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\work\workRWdensity'
    work = {
        'wxqsm': 'wxqmj',
        # 'wdqsm': 'wdqmj',
        # 'wzqsm': 'wzqmj'

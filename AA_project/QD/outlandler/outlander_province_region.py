...
def tableTransferToJson(conn, path):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    js = os.path.abspath(os.path.join(path, 'outlanderChina.js'))
    print '------------%s------------' % js
    fw = codecs.open(js, 'w', 'utf-8')
    fw.write('var outlanderData=[')
    fw.write('\r\n')
    cursor = conn.cursor()
    sql = 'select region,sum(outlander) from qd_province_outlander  group by region'
    print sql
    cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\outlander\outlander_region'
    if conn:
        print info

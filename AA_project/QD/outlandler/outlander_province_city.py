...
def tableTransferToJson(conn, path):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    js = os.path.abspath(os.path.join(path, 'outlandershandong.js'))
    print '------------%s------------' % js
    ...
    cursor = conn.cursor()
    city = "山东"
    sql = "select city,sum(outlander) from qd_province_outlander where region = '%s' group by city" % city.decode("utf-8")
    print sql
    cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\outlander\outlander_city'
    if conn:
        print info

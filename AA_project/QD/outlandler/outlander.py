...
def tableTransferToJson(conn, sqlTime, sqlJtqy, path):
    ...
    for qybh, qysm in sqlJtqy.items():
        ...
        for sqld in sqlTime:
            ...
            js = os.path.abspath(os.path.join(path_new, sqld_name + 'total.js'))
            print '------------%s------------' % js
            ...
            fw.write("\n")
            cursor = conn.cursor()
            sql = 'select %s,sum(outlander) from qd_outlander where data_month = %s and len(%s) > 0 group by %s' % (qysm, sqld, qysm, qysm)
            print sql
            cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\outlander\outlander_total'

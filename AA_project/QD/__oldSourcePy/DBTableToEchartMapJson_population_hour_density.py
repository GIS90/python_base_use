        ...
        for sqld in sqlDate:
            for pop in popType:
                ...
                for sqlh in range(0, 24, 1):
                    fw.write('"%d":[' % sqlh)
                    if qybh == 'xqbh':
                        sql = "select %s,convert(int,sum(%s)/mj_gl) from population_hour_popdensity where data_day='%s' and data_hour=%d and %s!=''  GROUP BY %s,mj_gl"\
                              % (qysm, pop, sqld, sqlh, qysm, qysm)
                    else:
                        sql = "select %s,sum(%s) from population_hour_jtqy where data_day='%s' and data_hour=%d  GROUP BY %s"\
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_js\population_hour_density'
    if conn:
        print info

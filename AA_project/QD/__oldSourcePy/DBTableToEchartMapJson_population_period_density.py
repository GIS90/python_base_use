            ...
            for (key, value) in sqlPeriod.items():
                ...
                for pop in popType:
                    ...
                    if pop == 'workder':
                        ...
                        pop_new = pop
                    js = os.path.join(path_new, (sqld_new + perKey + pop_new + '.js'))
                    ...
                    if qysm == 'xqsm':
                        sql = "select %s,convert(int,sum(%s)/mj_gl) from population_period_popdensity where data_day='%s' and period='%s' and %s!='' GROUP BY %s,mj_gl"%(qysm, pop, sqld, str(perValue).decode('utf-8'), qysm, qysm)
                    else:
                        sql = "select %s,sum(%s) from population_period_jtqy where data_day='%s' and period='%s' and %s!='' GROUP BY %s"%(qysm, pop, sqld, str(perValue).decode('utf-8'), qysm, qysm)
                    print sql
                    try:
                        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_js\population_period_density'
    if conn:
        print info

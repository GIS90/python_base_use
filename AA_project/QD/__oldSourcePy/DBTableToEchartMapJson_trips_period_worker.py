...
def tableTransferToJson(conn, sqlDate, sqlPeriod, jsonPath):
    ...
    for sqld in sqlDate:
        for (key, value) in sqlPeriod.items():
            ...
            js = os.path.join(jsonPath, (perKey + sqld_new + '.js'))
            ...
            FROM
            (
            select leftdqsm,arrdqsm,sum(worker) as nums
            from trips_period_worker_jtqy
...
if __name__ == '__main__':
    ...
    sqlPeriod = {'yj': ' 夜间',
                 ...
                 'wgf': ' 晚高峰',
                 'bt': ' 白天'}
    jsonPath = r'E:\data\json\trips_period_worker\big'

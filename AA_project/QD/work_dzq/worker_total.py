...
def tableTransferToJson(conn, sqlJtqy, path):
    ...
    for qybh, qysm in sqlJtqy.items():
        ...
        json = os.path.join(path_new, 'workRWtotal.js')
        ...
        fw.write("\n")
        if qybh == 'zqbh':
            sql = "select %s, worker from trips_upd_zq" % qysm
        elif qybh == 'dqbh':
            sql = "select %s, worker from trips_upd_dq" % qysm
        print sql
        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\work\workRW'
    if conn:
        print info

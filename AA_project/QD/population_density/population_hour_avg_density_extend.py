    ...
    for qybh, qysm in sqlJtqy.items():
        ...
        for wt in weekType:
            for sqld in sqlDate:
                for pop in popType:
                    nd = sqld[0:4] + '-' + sqld[4:]
                    js = os.path.join(path_new, (wt + nd + pop + '.js'))
                    ...
                    for sqlh in range(0, 24, 1):
                        fw.write('"%d":[' % sqlh)
                        if qybh == 'xqbh':
                            sql_main = "select %s,sum(%s),xqmj from qd_population_hour_avg where data_month='%s' and data_hour='%s' and workday = '%s' and %s!='' GROUP BY %s,xqmj" \
                            ...
                            sql = "select %s,sum(%s),zqmj from qd_population_hour_avg where data_month='%s' and data_hour='%s' and workday = '%s' and %s!=''  GROUP BY %s,zqmj" \
                                  % (qysm, pop, sqld, sqlh, wt, qysm, qysm)
                            print pop + ' : ' + sql
                            cursor.execute(sql)
                            rows = cursor.fetchall()
                            ...
                            sql = "select %s,sum(%s),dqmj from qd_population_hour_avg where data_month='%s' and data_hour='%s' and workday = '%s' and %s!=''  GROUP BY %s,dqmj" \
                                  % (qysm, pop, sqld, sqlh, wt, qysm, qysm)
                            print pop + ' : ' + sql
                            cursor.execute(sql)
                            rows = cursor.fetchall()
...
if __name__ == '__main__':
    ...
    path = r'E:\data\qd_data_js\data\population_density\population_hour_avg'
    if conn:
        print info

...
_task_id = {"create_customer_raw_data_table": 1,  # 创建客户原始数据表结构
            "upload_customer_raw_data": 2,  # 上传客户原始数据
            "transform_to_spatio_temporal_raw_data": 3,  # 原始数据转换成内部表结构数据
            "compute_raw_data_stat": 4,  # 计算原始数据统计信息
            "compute_people_distribution": 5,  # 计算人口数据量分布情况
            "compute_base_station_info": 6,  # 计算基站信息
            "download_base_station_info": 7,  # 下载基站信息
            "filter_data_with_range": 8,  # 过滤随机数据(噪点数据)
            "compute_filtered_data_stat": 9,  # 计算被过滤后的数据状态
            "compute_base_station_hour_summary": 10,  # 计算基站每小时粒度热度
            "download_base_station_hour_summary": 11,  # 下载基站每小时粒度热度
            "compute_uuid_cell_hour": 12  # 生成人的时空信息表
            }
